...
{$ENDIF}
  ...
  PasDoc_Languages,
  PasDoc_GenHtmlHelp, PasDoc_GenLatex,} {CustomProjectOptions,}
  ConsolProjectOptionsUnit, QFileCtrls, Contnrs;
...
type
  ...
  TfrmHelpGenerator = class(TForm)
    ...
    Label19: TLabel;
    edPasDocConsole: TEdit;
    btnPasDocConsole: TButton;
    Label20: TLabel;
    lbNavigator: TListBox;
    Splitter1: TSplitter;
    tabWebOptions: TTabSheet;
    ...
    Splitter2: TSplitter;
    Label21: TLabel;
    edHtmlHelpContents: TEdit;
    btnHtmlHelpConents: TButton;
    Label22: TLabel;
    clbSort: TCheckListBox;
    tabSpellChecking: TTabSheet;
    Panel10: TPanel;
    cbCheckSpelling: TCheckBox;
    comboSpellLanguages: TComboBox;
    Label23: TLabel;
    Label24: TLabel;
    memoCheckSpellingIgnoreWords: TMemo;
    edSpellUrl: TEdit;
    Label25: TLabel;
    Label26: TLabel;
    edGraphVizUrl: TEdit;
    ...
    procedure edPasDocConsoleChange(Sender: TObject);
    procedure lbNavigatorClick(Sender: TObject);
    procedure edHtmlHelpContentsChange(Sender: TObject);
    procedure btnHtmlHelpConentsClick(Sender: TObject);
    procedure comboLanguagesChange(Sender: TObject);
    procedure cbCheckSpellingClick(Sender: TObject);
    procedure comboSpellLanguagesChange(Sender: TObject);
    procedure memoCheckSpellingIgnoreWordsChange(Sender: TObject);
  private
    // @name is @true when the user has changed the project settings.
    // Otherwise it is @false.
    Changed: boolean;
    DefaultDirectives: TStringList;
    NavigateList: TObjectList;
    ...
    procedure GenerateNavigateList;
  ...
  THelpGeneratorProjectOptions = class(TConsolHelpProjectOptions)
  private
  protected
  ...
  T_ConsoleCLX_Ini = class(TCustomIniFile)
  protected
    function GetGraphVizDotLocation: string; override;
    procedure SetGraphVizDotLocation(const Value: string); override;
...
uses {PasDoc_Items,} ASpellLanguages, frmAboutUnit;
...
procedure TfrmHelpGenerator.GenerateNavigateList;
var
  Index: integer;
  Item: TNavigateItem;
begin
  lbNavigator.Items.Clear;
  NavigateList.Clear;
  for Index := 0 to pcMain.PageCount -1 do
  begin
    if pcMain.Pages[Index].Enabled then
    begin
      lbNavigator.Items.Add(pcMain.Pages[Index].Caption);
      Item := TNavigateItem.Create;
      Item.PageIndex := Index;
      NavigateList.Add(Item);
    end;
  end;
end;
...
begin
  ...
  NavigateList:= TObjectList.Create;
  GenerateNavigateList;

  comboSpellLanguages.Items := LanguageMap;
  ...
  edPasDocConsoleChange(nil);
  ReadIniFile(T_ConsoleCLX_Ini);

{$IFDEF LINUX}
  odDotLocation.Filter := '(dot)|dot';
  ProgramDialog.Filter := '';
{$ENDIF}

  THelpGeneratorProjectOptions.DefaultDefines(memoDefines.Lines);

  comboLanguages.Items.Capacity := MaxLanguages + 1;
  for LanguageIndex := 0 to MaxLanguages do
  begin
    comboLanguages.Items.Add(Languages[LanguageIndex].Name);
  end;
  ...
  begin
    ...
    begin
      ...
      OpenAFile(OpenDialog2.FileName);
    end;
  end;
end;
...
begin
  if not FileExists(edPasDocConsole.Text) then
  begin
    pcMain.ActivePage := tabLocations;
    MessageDlg('The location of PasDoc consol has not been specified correctly.',
      mtError, [mbOK], 0);
    Exit;
  end;
...
{$ENDIF}
  ...
  try
    ...
    try
      Settings.RunPasDocConsol(ShowPasDocMessage);
      Settings.RunVizGraph(ShowPasDocMessage);
    finally
      Settings.Free;
    end;

    if cbCheckSpelling.Checked and (MisspelledWords.Count > 0) then
    begin
    ...
    begin
      MemoMessages.Lines.Add('');
      MemoMessages.Lines.Add('Starting web browser');
      Application.ProcessMessages;
      URL := FileToURL(edOutput.Text + PathDelim +
        'index.html');
...
procedure TfrmHelpGenerator.OpenAFile(const FileName: string);
var
  Settings: THelpGeneratorProjectOptions;
begin
  ...
  Settings := THelpGeneratorProjectOptions.Create(nil);
  try
    Settings.ReadFromComponentFile(FileName);
    Caption := ExtractFileName(FileName) + ': Help Generator';
  finally
    Settings.Free;
  end;
  Changed := False;
...
begin
  ...
  try
    ...
    memoCheckSpellingIgnoreWords.Lines.Clear;
    MisspelledWords.Clear;
...
begin
  ...
  tabWebOptions.Enabled := (comboGenerateFormat.ItemIndex in [0, 1]);

  edHtmlHelpContents.Enabled := comboGenerateFormat.ItemIndex = 1;
  btnHtmlHelpConents.Enabled := edHtmlHelpContents.Enabled;
  if edHtmlHelpContents.Enabled then
  begin
    edHtmlHelpContents.Color := clBase;
  end
  else
  begin
    edHtmlHelpContents.Color := clButton;
  end;    
...
{  memoHeader.Enabled := (comboGenerateFormat.ItemIndex in [0, 1]);
  ...
  comboLatexGraphics.Enabled := tabLatexOptions.Enabled;
...
{function THelpGeneratorProjectOptions.GetHtmlDocGeneratorComponent:
  THtmlDocGenerator;
begin
  result := nil;//frmHelpGenerator.HtmlDocGenerator;
end;

function THelpGeneratorProjectOptions.GetHTMLHelpDocGeneratorComponent:
  THTMLHelpDocGenerator;
begin
  result := nil;//frmHelpGenerator.HTMLHelpDocGenerator;
end; }

function THelpGeneratorProjectOptions.GetHtmlHelpContents: string;
begin
  result := frmHelpGenerator.edHtmlHelpContents.Text;
end;
...
function THelpGeneratorProjectOptions.GetLanguageString: string;
begin
  result := frmHelpGenerator.comboLanguages.Items[frmHelpGenerator.comboLanguages.ItemIndex];
end;
...
begin
  result := nil;//frmHelpGenerator.PasDoc1;
end; }

function THelpGeneratorProjectOptions.GetPasDocConsoleLocation: string;
begin
  result := frmHelpGenerator.edPasDocConsole.Text;
end;
...
function THelpGeneratorProjectOptions.GetSortOptions: TSortOptions;
var
  index: TSortOption;
begin
  result := [];
  for index := Low(TSortOption) to High(TSortOption) do
  begin
    if frmHelpGenerator.clbSort.Checked[Ord(index)] then
    begin
...
{function THelpGeneratorProjectOptions.GetTexDocGeneratorComponent:
  TTexDocGenerator;
begin
  result := nil;//frmHelpGenerator.TexDocGenerator;
end; }

function THelpGeneratorProjectOptions.GetSpellCheckIgnoreWords: TStrings;
begin
  result := frmHelpGenerator.memoCheckSpellingIgnoreWords.Lines;
end;

function THelpGeneratorProjectOptions.GetSpellChecking: boolean;
begin
  result := frmHelpGenerator.cbCheckSpelling.Checked;
end;

function THelpGeneratorProjectOptions.GetSpellCheckLanguage: string;
begin
  result := frmHelpGenerator.comboLanguages.Text;
end;

function THelpGeneratorProjectOptions.GetSpellCheckLanguageCode: string;
var
  Item: TASpellLangMap;
begin
  if frmHelpGenerator.comboSpellLanguages.ItemIndex >= 0 then
  begin
    Item := frmHelpGenerator.comboSpellLanguages.Items.Objects[
      frmHelpGenerator.comboSpellLanguages.ItemIndex] as TASpellLangMap;
    result := Item.Code;
  end
  else
  begin
...
procedure THelpGeneratorProjectOptions.SetHtmlHelpContents(
  const AValue: string);
begin
  frmHelpGenerator.edHtmlHelpContents.Text := AValue;
end;
...
procedure THelpGeneratorProjectOptions.SetLanguageString(
  const Value: string);
begin
  frmHelpGenerator.comboLanguages.ItemIndex :=
    frmHelpGenerator.comboLanguages.Items.IndexOf(Value);
  frmHelpGenerator.comboLanguagesChange(nil);
end;
...
procedure THelpGeneratorProjectOptions.SetSortOptions(
...
begin
  for index := Low(TSortOption) to High(TSortOption) do
  begin
    frmHelpGenerator.clbSort.Checked[Ord(index)] := Index in Value;
  end;
end;
...
procedure THelpGeneratorProjectOptions.SetSpellCheckIgnoreWords(
  const Value: TStrings);
begin
  frmHelpGenerator.memoCheckSpellingIgnoreWords.Lines := Value;

end;

procedure THelpGeneratorProjectOptions.SetSpellChecking(
  const Value: boolean);
begin
  frmHelpGenerator.cbCheckSpelling.Checked := Value;
end;

procedure THelpGeneratorProjectOptions.SetSpellCheckLanguage(
  const Value: string);
var
  Index: integer;
begin
  Index := frmHelpGenerator.comboSpellLanguages.Items.IndexOf(Value);
  frmHelpGenerator.comboSpellLanguages.ItemIndex := Index;
end;
...
function T_ConsoleCLX_Ini.GetBrowserLocation: string;
begin
  result := frmHelpGenerator.edBrowser.Text;
end;

function T_ConsoleCLX_Ini.GetGraphVizDotLocation: string;
begin
  result := frmHelpGenerator.edGraphVizDotLocation.Text;
end;

function T_ConsoleCLX_Ini.GetPasDocConsoleLocation: string;
begin
  result := frmHelpGenerator.edPasDocConsole.Text;
end;

procedure T_ConsoleCLX_Ini.SetBrowserLocation(const Value: string);
begin
  frmHelpGenerator.edBrowser.Text := Value;
end;

procedure T_ConsoleCLX_Ini.SetGraphVizDotLocation(const Value: string);
begin
  frmHelpGenerator.edGraphVizDotLocation.Text := Value;
end;

procedure TfrmHelpGenerator.btnBrowerClick(Sender: TObject);
var
  ed: TEdit;
begin
  ed := nil;
  if Sender = btnBrower then
  begin
    ed := edBrowser;
  end
  else if Sender = btnPasDocConsole then
  begin
    ed := edPasDocConsole;
  end
  else
  ...
  ProgramDialog.FileName := ed.Text;
  if ProgramDialog.Execute then
  begin
    ed.Text := ProgramDialog.FileName;
  end;

end;

procedure T_ConsoleCLX_Ini.SetPasDocConsoleLocation(const Value: string);
begin
  frmHelpGenerator.edPasDocConsole.Text := Value;
  frmHelpGenerator.edPasDocConsoleChange(nil);
end;

procedure TfrmHelpGenerator.ShowPasDocMessage(const Text: string);
const
...
procedure TfrmHelpGenerator.edPasDocConsoleChange(Sender: TObject);
begin
  if FileExists(edPasDocConsole.Text) then
  begin
    edPasDocConsole.Color := clBase;
  end
  else
  begin
    edPasDocConsole.Color := clRed;
  end;

end;

procedure TfrmHelpGenerator.lbNavigatorClick(Sender: TObject);
var
  Item: TNavigateItem;
begin
  if lbNavigator.ItemIndex >= 0 then
  begin
    Item := NavigateList[lbNavigator.ItemIndex] as TNavigateItem;
    pcMain.ActivePageIndex := Item.PageIndex;
  end;
end;

procedure TfrmHelpGenerator.edHtmlHelpContentsChange(Sender: TObject);
begin
  Changed := True;
  if edHtmlHelpContents.Enabled then
  begin
    if FileExists(edHtmlHelpContents.Text) then
    begin
      edHtmlHelpContents.Color := clBase;
    end
    else
    begin
      edHtmlHelpContents.Color := clRed;
    end;
  end;
end;

procedure TfrmHelpGenerator.btnHtmlHelpConentsClick(Sender: TObject);
begin
  odExtraFiles.FileName := edHtmlHelpContents.Text;
  if odExtraFiles.Execute then
  begin
    edHtmlHelpContents.Text := odExtraFiles.FileName;
    edHtmlHelpContentsChange(nil);
  end;
end;
...
begin
  Changed := True;
  Lang := comboLanguages.Text;
  SpacePos := Pos(' ', Lang);
  if SpacePos > 0 then
  begin
    Lang := Copy(Lang, 1, SpacePos-1);
  end;
  if Lang = 'Brasilian' then
  begin
    Lang := 'Portuguese';
  end
  else if Lang = 'Catalan' then
  begin
    Lang := 'Catalan / Valencian';
  end;
  LangPos := comboSpellLanguages.Items.IndexOf(Lang);
  if LangPos >= 0 then
  begin
    comboSpellLanguages.ItemIndex := LangPos
  end;
end;

procedure TfrmHelpGenerator.cbCheckSpellingClick(Sender: TObject);
begin
  Changed := True;
  comboSpellLanguages.Enabled := cbCheckSpelling.Checked;
  memoCheckSpellingIgnoreWords.Enabled := cbCheckSpelling.Checked;
  if comboSpellLanguages.Enabled then
  begin
    comboSpellLanguages.Color := clBase;
    memoCheckSpellingIgnoreWords.Color := clBase;
  end
  else
  begin
    comboSpellLanguages.Color := clButton;
    memoCheckSpellingIgnoreWords.Color := clButton;
  end;
end;

procedure TfrmHelpGenerator.comboSpellLanguagesChange(Sender: TObject);
begin
  Changed := True;
end;

procedure TfrmHelpGenerator.memoCheckSpellingIgnoreWordsChange(
  Sender: TObject);
begin
  Changed := True;
end;
...
initialization
  ...
  Classes.RegisterClass(T_ConsoleCLX_Ini);
